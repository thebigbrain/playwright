...
export async function launchApp(
  ...
  options: {
    ...
    persistentContextOptions?: Parameters<
  ...
  if (browserType.name() === "chromium") {
    args.push(
      ...
      ...(options.windowPosition
        ? [
            `--window-position=${options.windowPosition.x},${options.windowPosition.y}`,
          ]
        : []),
      "--test-type="
    );
...
async function installAppIcon(page: Page) {
  const icon = await fs.promises.readFile(
    require.resolve("./chromium/appIcon.png")
  );
...
export async function syncLocalStorageWithSettings(
  page: Page,
  appName: string
) {
  ...
  await page.addInitScript(
    `(${String((settings: any) => {
      ...
      (window as any).saveSettings = () => {
        (window as any)._saveSerializedSettings(
